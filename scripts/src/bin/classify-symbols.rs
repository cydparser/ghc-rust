...
pub fn main() {
    ...
    let symbols = find_places(ghc_path, visitor.symbols.iter().map(|s| s.as_ref()));
...
fn find_places<'a, P: AsRef<Path>>(
    path: P,
    syms: impl IntoIterator<Item = &'a str>,
) -> BTreeMap<String, Places> {
    fn search(path: &Path, syms_regex: &str, args: &[&str]) -> BTreeMap<String, Places> {
