...
pub(crate) struct NonmovingSegment {
    ...
    pub(crate) bitmap: [uint8_t; 0],
}

pub(crate) type nonmoving_block_idx = uint16_t;
...
pub(crate) struct NonmovingHeap {
    ...
    pub(crate) n_caps: c_uint,
    pub(crate) sweep_list: *mut NonmovingSegment,
}

pub(crate) const NONMOVING_SEGMENT_BITS: c_ulonglong = 15 as c_ulonglong;

pub(crate) const NONMOVING_SEGMENT_MASK: uintptr_t =
    ((1 as c_int as uintptr_t) << NONMOVING_SEGMENT_BITS).wrapping_sub(1 as c_int as uintptr_t);

pub(crate) const NONMOVING_SEGMENT_SIZE: uintptr_t =
    (1 as c_int as uintptr_t) << NONMOVING_SEGMENT_BITS;

pub(crate) const NONMOVING_SEGMENT_BLOCKS: uintptr_t =
    NONMOVING_SEGMENT_SIZE.wrapping_div(BLOCK_SIZE as uintptr_t);

pub(crate) const NONMOVING_ALLOCA0: c_int = 8 as c_int;

pub(crate) const NONMOVING_ALLOCA_DIVIDE_SHIFT: c_int = 32 as c_int;

#[inline]
pub(crate) unsafe fn log2_ceil(mut x: c_ulong) -> c_ulong {
    return (size_of::<c_ulong>() as c_ulong)
        .wrapping_mul(8 as c_ulong)
        .wrapping_sub(x.wrapping_sub(1 as c_ulong).leading_zeros() as i32 as c_ulong);
}
...
#[inline]
pub(crate) unsafe fn nonmovingAllocatorForSize(mut block_size: uint16_t) -> uint8_t {
    if block_size as c_int - NONMOVING_ALLOCA0
        < nonmoving_alloca_dense_cnt as c_int * size_of::<StgWord>() as uint16_t as c_int
    {
        return ((block_size as c_int - NONMOVING_ALLOCA0) as usize)
            .wrapping_div(size_of::<StgWord>() as usize) as uint8_t;
    } else {
        return log2_ceil(block_size as c_ulong)
            .wrapping_sub(log2_ceil(
                (NONMOVING_ALLOCA0 as c_ulong).wrapping_add(
                    (size_of::<StgWord>() as c_ulong)
                        .wrapping_mul(nonmoving_alloca_dense_cnt as c_ulong),
                ),
            ))
            .wrapping_add(nonmoving_alloca_dense_cnt as c_ulong) as uint8_t;
    };
}

#[inline]
pub(crate) unsafe fn nonmovingSegmentBlockSize(mut seg: *mut NonmovingSegment) -> c_uint {
    return nonmovingSegmentAllocator(seg).block_size as c_uint;
}
...
pub(crate) unsafe fn nonmovingPushActiveSegment(mut seg: *mut NonmovingSegment) {
    let mut alloc: *mut NonmovingAllocator =
        nonmovingHeap.allocators.offset((nonmovingAllocatorForSize
            as unsafe extern "C" fn(uint16_t) -> uint8_t)(
...
#[inline]
pub(crate) unsafe fn nonmovingBlockCount(mut block_size: uint16_t) -> c_uint {
    let mut segment_data_size =
        NONMOVING_SEGMENT_SIZE.wrapping_sub(size_of::<NonmovingSegment>() as uintptr_t) as c_uint;
    segment_data_size =
        segment_data_size.wrapping_sub(segment_data_size.wrapping_rem(SIZEOF_VOID_P as c_uint));

    let mut block_count =
        segment_data_size.wrapping_div((block_size as c_int + 1 as c_int) as c_uint);

    return block_count;
}

#[inline]
pub(crate) unsafe fn nonmovingSegmentBlockCount(mut seg: *mut NonmovingSegment) -> c_uint {
    return nonmovingSegmentAllocator(seg).block_count as c_uint;
}

#[inline]
pub(crate) unsafe fn nonmovingSegmentGetBlock_(
    mut seg: *mut NonmovingSegment,
    mut block_size: uint16_t,
    mut block_count: uint16_t,
    mut i: nonmoving_block_idx,
) -> *mut c_void {
    let mut bitmap_size: W_ =
        (block_count as usize).wrapping_mul(size_of::<uint8_t>() as usize) as W_;
    ...
    return data.wrapping_add((i as c_int * block_size as c_int) as W_) as *mut c_void;
}
...
pub(crate) unsafe fn nonmovingSegmentGetBlock(
    ...
    return nonmovingSegmentGetBlock_(
        seg,
        nonmovingSegmentBlockSize(seg) as uint16_t,
        nonmovingSegmentBlockCount(seg) as uint16_t,
        i,
    );
}
...
pub(crate) unsafe fn nonmovingGetSegment_unchecked(mut p: StgPtr) -> *mut NonmovingSegment {
    let mask: uintptr_t = !NONMOVING_SEGMENT_MASK;

    return (p as uintptr_t & mask) as *mut NonmovingSegment;
}
...
pub(crate) unsafe fn nonmovingIsInSegment(mut p: StgPtr) -> bool {
    ...
    return p as W_ >= mblock_address_space.0.begin
        ...
        && (*bd).flags as c_int & BF_NONMOVING != 0
        && (*bd).flags as c_int & BF_LARGE == 0;
}
...
pub(crate) unsafe fn nonmovingSegmentDivideBySize(
    ...
    return ((x as StgWord64)
        ...
        >> NONMOVING_ALLOCA_DIVIDE_SHIFT) as uint16_t;
}

#[inline]
pub(crate) unsafe fn nonmovingGetBlockIdx(mut p: StgPtr) -> nonmoving_block_idx {
    let mut seg = nonmovingGetSegment(p);
    let mut blk0: ptrdiff_t = nonmovingSegmentGetBlock(seg, 0 as nonmoving_block_idx) as ptrdiff_t;
    let mut offset: ptrdiff_t = p as ptrdiff_t - blk0;

    return nonmovingSegmentDivideBySize(seg, offset as uint16_t);
}

#[inline]
pub(crate) unsafe fn nonmoving_first_sparse_allocator_size() -> uint16_t {
    return log2_ceil(
        (NONMOVING_ALLOCA0 as c_ulong)
            .wrapping_add(
                ((nonmoving_alloca_dense_cnt as c_int - 1 as c_int) as c_ulong)
                    .wrapping_mul(size_of::<StgWord>() as c_ulong),
            )
...
pub(crate) unsafe fn nonmovingSetMark(mut seg: *mut NonmovingSegment, mut i: nonmoving_block_idx) {
    *(&raw mut (*seg).bitmap as *mut uint8_t).offset(i as isize) = nonmovingMarkEpoch;
}
...
pub(crate) unsafe fn nonmovingGetMark(
    ...
    return *(&raw mut (*seg).bitmap as *mut uint8_t).offset(i as isize);
}
...
#[inline]
pub(crate) unsafe fn nonmovingGetClosureMark(mut p: StgPtr) -> uint8_t {
    let mut seg = nonmovingGetSegment(p);
...
pub(crate) unsafe fn nonmovingClosureMarkedThisCycle(mut p: StgPtr) -> bool {
    return nonmovingGetClosureMark(p) as c_int == nonmovingMarkEpoch as c_int;
}
...
pub(crate) unsafe fn nonmovingSegmentBeingSwept(mut seg: *mut NonmovingSegment) -> bool {
    ...
    return (*seginfo).next_free_snap as c_uint >= n;
}
...
pub(crate) unsafe fn nonmovingClosureBeingSwept(mut p: *mut StgClosure) -> bool {
    ...
    if p as W_ >= mblock_address_space.0.begin && (p as W_) < mblock_address_space.0.end {
        if (*bd).flags as c_int & BF_NONMOVING_SWEEPING != 0 {
            return r#true != 0;
        } else if (*bd).flags as c_int & BF_NONMOVING != 0 {
...
pub(crate) unsafe fn isNonmovingClosure(mut p: *mut StgClosure) -> bool {
    return RtsFlags.GcFlags.useNonmoving as c_int != 0
        && (!(p as W_ >= mblock_address_space.0.begin && (p as W_) < mblock_address_space.0.end)
            || (*Bdescr(p as StgPtr)).flags as c_int & BF_NONMOVING != 0);
}
...
static mut nonmovingHeap: NonmovingHeap = NonmovingHeap {
    allocators: null::<NonmovingAllocator>() as *mut NonmovingAllocator,
    free: null::<NonmovingSegment>() as *mut NonmovingSegment,
    saved_free: null::<NonmovingSegment>() as *mut NonmovingSegment,
    n_caps: 0,
    sweep_list: null::<NonmovingSegment>() as *mut NonmovingSegment,
};

static mut nonmovingMarkEpoch: uint8_t = 1 as uint8_t;

static mut nonmoving_alloca_dense_cnt: uint8_t = 0;

static mut nonmoving_alloca_cnt: uint8_t = 0;

unsafe fn nonmovingBumpEpoch() {
    nonmovingMarkEpoch = (if nonmovingMarkEpoch as c_int == 1 as c_int {
        2 as c_int
    } else {
        1 as c_int
    }) as uint8_t;
...
static mut nonmoving_segment_live_words: memcount = 0 as memcount;

static mut sync_phase_marking_budget: MarkBudget = 200000 as MarkBudget;
...
unsafe fn cmp_segment_ptr(mut x: *const c_void, mut y: *const c_void) -> c_int {
...
unsafe fn nonmovingPruneFreeSegmentList() {
    if TRACE_nonmoving_gc as c_long != 0 {
        trace_(b"Pruning free segment list.\0" as *const u8 as *const c_char as *mut c_char);
    }
    ...
    let mut sorted = stgMallocBytes(
        (size_of::<*mut NonmovingSegment>() as size_t).wrapping_mul(length),
        b"sorted free segment list\0" as *const u8 as *const c_char as *mut c_char,
    ...
    qsort(
        ...
        length,
        size_of::<*mut NonmovingSegment>() as size_t,
        Some(cmp_segment_ptr as unsafe extern "C" fn(*const c_void, *const c_void) -> c_int),
    );

    let mut new_length: size_t = 0 as size_t;
    let mut free_in_megablock: size_t = 0 as size_t;
    let mut i_0: size_t = 0 as size_t;

    while i_0 < length {
        free_in_megablock = 1 as size_t;
    ...
    let mut pruned_segments: size_t = length.wrapping_sub(new_length);
    (*oldest_gen).n_blocks = (*oldest_gen)
        .n_blocks
        .wrapping_sub(pruned_segments.wrapping_mul(NONMOVING_SEGMENT_BLOCKS as size_t) as memcount);
    (*oldest_gen).n_words = (*oldest_gen)
        .n_words
        .wrapping_sub(pruned_segments.wrapping_mul(NONMOVING_SEGMENT_SIZE as size_t) as memcount);
    nonmovingHeap.saved_free = null_mut::<NonmovingSegment>();

    if DEBUG_RTS != 0 && RtsFlags.DebugFlags.nonmoving_gc as c_long != 0 {
        trace_(
            b"Pruned %d free segments, leaving %d on the free segment list.\0" as *const u8
    ...
    traceNonmovingPrunedSegments(pruned_segments as uint32_t, new_length as uint32_t);

    if TRACE_nonmoving_gc as c_long != 0 {
        trace_(
            b"Finished pruning free segment list.\0" as *const u8 as *const c_char as *mut c_char,
        );
    }
}

unsafe fn nonmovingInitAllocator(mut alloc: *mut NonmovingAllocator, mut block_size: uint16_t) {
    *alloc = NonmovingAllocator {
        ...
        block_division_constant: (-(1 as c_int) as StgWord32)
...
unsafe fn nonmovingInitAllocators() {
    nonmoving_alloca_dense_cnt = RtsFlags.GcFlags.nonmovingDenseAllocatorCount as uint8_t;

    let mut first_sparse_allocator = nonmoving_first_sparse_allocator_size();
    let mut nonmoving_alloca_sparse_cnt: uint16_t = log2_ceil(NONMOVING_SEGMENT_SIZE as c_ulong)
        .wrapping_sub(first_sparse_allocator as c_ulong)
        as uint16_t;
    nonmoving_alloca_cnt =
        (nonmoving_alloca_dense_cnt as c_int + nonmoving_alloca_sparse_cnt as c_int) as uint8_t;

    nonmovingHeap.allocators = stgMallocBytes(
        (size_of::<NonmovingAllocator>() as size_t).wrapping_mul(nonmoving_alloca_cnt as size_t),
        b"allocators array\0" as *const u8 as *const c_char as *mut c_char,
    ) as *mut NonmovingAllocator;

    let mut i = 0 as c_uint;

    while i < nonmoving_alloca_dense_cnt as c_uint {
    ...
    let mut i_0 = nonmoving_alloca_dense_cnt as c_uint;

    while i_0 < nonmoving_alloca_cnt as c_uint {
        let mut block_size: uint16_t = ((1 as c_int)
            << i_0
                .wrapping_add(first_sparse_allocator as c_uint)
                .wrapping_sub(nonmoving_alloca_dense_cnt as c_uint))
            as uint16_t;
...
unsafe fn nonmovingPrepareMark() {
    ...
    nonmovingHeap.n_caps = n_capabilities as c_uint;
    nonmovingBumpEpoch();

    let mut alloca_idx = 0 as c_int;

    while alloca_idx < nonmoving_alloca_cnt as c_int {
        let mut alloca: *mut NonmovingAllocator =
            nonmovingHeap.allocators.offset(alloca_idx as isize) as *mut NonmovingAllocator;

        let mut cap_n: uint32_t = 0 as uint32_t;

        while cap_n < nonmovingHeap.n_caps as uint32_t {
    ...
    while !bd_0.is_null() {
        ...
        (*bd_0).flags = ((*bd_0).flags as c_int | BF_NONMOVING_SWEEPING) as StgWord16;
        (*bd_0).flags = ((*bd_0).flags as c_int & !BF_MARKED) as StgWord16;
    ...
    (*oldest_gen).n_large_words = 0 as memcount;
    (*oldest_gen).n_large_blocks = 0 as memcount;
    nonmoving_segment_live_words = 0 as memcount;

    let mut bd_1 = nonmoving_compact_objects;

    while !bd_1.is_null() {
        (*bd_1).flags = ((*bd_1).flags as c_int & !BF_MARKED) as StgWord16;
    ...
    while !bd_2.is_null() {
        ...
        (*bd_2).flags = ((*bd_2).flags as c_int | BF_NONMOVING_SWEEPING) as StgWord16;
        (*bd_2).flags = ((*bd_2).flags as c_int & !BF_MARKED) as StgWord16;
    ...
    (*oldest_gen).n_compact_blocks = 0 as memcount;
...
unsafe fn nonmovingCollect(
    ...
    concurrent = r#false != 0;

    if TRACE_nonmoving_gc as c_long != 0 {
        trace_(b"Starting nonmoving GC preparation\0" as *const u8 as *const c_char as *mut c_char);
    }

    resizeGenerations();
    nonmovingPrepareMark();

    let mut mark_queue = stgMallocBytes(
        size_of::<MarkQueue>() as size_t,
        b"mark queue\0" as *const u8 as *const c_char as *mut c_char,
    ) as *mut MarkQueue;
    ...
    if TRACE_nonmoving_gc as c_long != 0 {
        trace_(b"Marking roots for nonmoving GC\0" as *const u8 as *const c_char as *mut c_char);
    }
    ...
    if TRACE_nonmoving_gc as c_long != 0 {
        trace_(
            b"Finished marking roots for nonmoving GC\0" as *const u8 as *const c_char
                as *mut c_char,
        );
    }
    ...
    let mut n_0: uint32_t = 0 as uint32_t;
    ...
    if DEBUG_RTS != 0 && RtsFlags.DebugFlags.nonmoving_gc as c_long != 0 {
        trace_(
            b"%d new nonmoving weaks\0" as *const u8 as *const c_char as *mut c_char,
            n_0,
        );
    }
    ...
    *weaks = nonmoving_weak_ptr_list;
    ...
    if TRACE_nonmoving_gc as c_long != 0 {
        trace_(b"Finished nonmoving GC preparation\0" as *const u8 as *const c_char as *mut c_char);
    }

    if getSchedState() as c_uint != SCHED_RUNNING as c_int as c_uint {
        concurrent = r#false != 0;
    }
    ...
    if concurrent {
        ...
        nonmovingMark_(mark_queue, dead_weaks, resurrected_threads, r#false != 0);
    };
}
...
unsafe fn nonmovingStartConcurrentMark(mut roots: *mut MarkQueue) -> ! {
    barf(
        b"nonmovingStartConcurrentMark: Not supported in non-threaded RTS\0" as *const u8
            as *const c_char,
    );
}
...
unsafe fn nonmovingMark_(
    ...
    if DEBUG_RTS != 0 && RtsFlags.DebugFlags.nonmoving_gc as c_long != 0 {
        trace_(b"Starting mark...\0" as *const u8 as *const c_char as *mut c_char);
    }
    ...
    if DEBUG_RTS != 0 && RtsFlags.DebugFlags.nonmoving_gc as c_long != 0 {
        trace_(
            b"Done marking, resurrecting threads before releasing capabilities\0" as *const u8
                as *const c_char as *mut c_char,
        );
    }

    if concurrent {
        scheduleFinalizers(getCapability(0 as uint32_t), *dead_weaks);
    ...
    *threads = nonmoving_threads;
    ...
    (*oldest_gen).n_old_blocks = 0 as memcount;
    resizeGenerations();
    traceConcSweepBegin();
    ...
    if DEBUG_RTS != 0 && RtsFlags.DebugFlags.nonmoving_gc as c_long != 0 {
        trace_(b"Finished sweeping.\0" as *const u8 as *const c_char as *mut c_char);
    }
